if(!document.getElementById("trak_splash")){ 
  ...
  splash.innerHTML=`<div style="font-size:60px;font-weight:bold;color:#ff0;text-shadow:0 0 15px #ff0;">Trakinas</div>
...
function initGui(){
  ...
  gui.innerHTML=`
    ...
    <input type="range" id="trak_speed" min="1" max="50" step="1" value="50"/> 
    <div>Velocidade: <span id="trak_speed_val">50</span>ms</div> 
    <button id="auto_type">Auto Digitar</button>
    <button id="force_copy">Forçar Copia (mobile)</button>
  ...
  const copyBtn=document.getElementById("force_copy");
  ...
  // === Auto Digitar com countdown 2.5s ===
  let targetField = null;
  const focusListener = e => { targetField = e.target; };
  document.addEventListener("focus", focusListener, true);

  autoBtn.onclick = function(){
    if(!textInput.value){
      status.textContent = "⚠️ Digite algo primeiro!";
      setTimeout(()=>{status.textContent="";},2000);
      return;
    }

    status.textContent = "⏳ Selecione o campo em 2.5s";
    let countdown = 2.5;
  ...
  // === Forçar Copia do texto selecionado na página ===
  copyBtn.onclick = function(){
    const selection = window.getSelection().toString();
    if(!selection){
      status.textContent="⚠️ Nenhum texto selecionado!";
      setTimeout(()=>{status.textContent="";},2000);
      return;
    }
    navigator.clipboard.writeText(selection).then(()=>{
      ...
      status.textContent="⚠️ Falha ao copiar!";
      setTimeout(()=>{status.textContent="";},2000);
    });
  };
  ...
  topbar.addEventListener("mousedown",e=>{
    dragging=true; dx=e.clientX-gui.offsetLeft; dy=e.clientY-gui.offsetTop;
  });
  document.addEventListener("mousemove",e=>{
    if(dragging){ gui.style.left=e.clientX-dx+"px"; gui.style.top=e.clientY-dy+"px"; }
  });
  document.addEventListener("mouseup",()=>dragging=false);
  topbar.addEventListener("touchstart",e=>{
    dragging=true; dx=e.touches[0].clientX-gui.offsetLeft; dy=e.touches[0].clientY-gui.offsetTop;
  },{passive:true});
  document.addEventListener("touchmove",e=>{
    if(dragging){ gui.style.left=e.touches[0].clientX-dx+"px"; gui.style.top=e.touches[0].clientY-dy+"px"; }
  },{passive:true});
